# 人生余额 · Personal Survival Dashboard

纯前端静态网站，本地换算「我还能活多久」与「钱还能撑多久」。所有数据保存在浏览器 `localStorage`，**不上传任何服务器**。

> 本项目仅为个人计算工具，不提供任何金融、投资、医疗或寿命预测建议。

**当前版本：`v1.3.2`**（首屏两卡「小字行 + 高亮条」对齐；此前 v1.3.1 做大屏自适应、v1.3.0 做统一栅格/一屏/手机/OG 卡，仓库名仍沿用 `personal-survival-dashboard`）
...
| 版本 | 内容 |
| --- | --- |
| `v1.3.2` | 首屏两卡「小字行 + 高亮条」对齐。此前卡 A 结构是「大数字 → 小字行 → 高亮条」，卡 B 是「大数字 → 高亮条 → 小字行」——结论行在条**下面**，把卡 B 的高亮条整体顶高约一行，两条黄条左右错开。改法：①结论行 `.card-sub-note` 移到卡 B 高亮条**上方**，两卡结构一致；②`.card-sub-note` 字号由 `--fs-xs` 提为 `--fs-sm`，与卡 A 的 `.elapsed-line` 同行高；③`.stat-card > :last-child { margin-top: auto }` 把副信息区吸到卡底，两卡等高时高亮条底边必然齐平（单列断点无多余空间，此规则自动失效）。复测 90 PASS / 0 FAIL：7 档视口（2560×1440 / 1920×1080 / 1600×900 / 1440×900 / 1440×800 / 1280×800 / 1024×768）两条高亮条 Δtop=Δbottom=0 |
| `v1.3.1` | 大屏（2K/4K）适配：v1.3.0 把桌面尺寸按「1440×800 一屏」写成了固定值，在 2560×1440 这类窗口上内容会缩成中间一小块（宽占 47%、高占 58%）、下方大片留白。改为 **`clamp(紧凑值, calc(紧凑值 + (100vh - 800px) * 系数), 舒适值)`**：窗口高 800px 时结果与 v1.3.0 **逐像素相同**（一屏保证不变），越高则尺寸线性回升（2560×1440 下容器 1200→1520、主数字 36→59px、卡内边距 14→28px、进度环 132→192px）。另加宽屏两档 `--max-width`：≥1600px → 1320px、≥2200px → 1520px。复测 9 视口：1440×800 / 1280×800 / 1600×900 / 1920×1200 / 2160×1440 / 2560×1440 / 3840×2160 全部零滚动、零横向溢出 |
...
| 版本 | 内容 |
|---|---|
| `v1.2.1` | 极简化：删除生命模块的「时间详情」卡（当前年龄 / 出生日期 / 已度过 / 剩余时间 / 下次生日 / 退休日 六行），「已度过」时长上移为卡 A 主数字下方一行小字（`已过 36 年 253 天`）；配套清理 `.details-card / .detail-row / .detail-sub / .tick-text` 死样式与 `tickingRemaining` 死代码 |
| `v1.2.0` | 首屏趣味化：品牌更名「人生余额」、两张卡改为「我还能活多久 / 钱还能撑多久」、新增高亮条（剩余年天放大高亮 + 秒级时钟 + GG 终点日期 + 存款耗尽日 + 谁先到结论）；免责声明与占位符同步更新 |
